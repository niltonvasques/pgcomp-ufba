...
\begin{frame}{Introdução}
  \begin{itemize}
    ...
    \item<2 -> Os dados por sua vez, estão armazenados em diversos tipos e formatos, sendo muitas
      vezes \alert{não estruturados}.
  \end{itemize}
  \begin{examples}<3 ->
...
\begin{frame}{Introdução}
  \begin{itemize}
    \item Dados estruturados já possuem mecanismos eficientes de armazenamento e recuperação.
    \item \alert{Documentos textuais} são recuperados através de Sistemas de Recuperação da
...
\begin{frame}{Introdução}
  ...
  processar e extrair padrões dos dados. 
